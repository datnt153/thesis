...
def main(args):
    ...
    def init_model(accelerator, model_name, use_pose):
        with accelerator.main_process_first():
            model = Model(model_name = model_name, use_pose=use_pose)
    ...
    def train_epoch(loader, model, optimizer, epoch, mixup=True, accelerator=None):
        losses = AverageMeter()
    ...
    for view in views :
        ...
        run = wandb.init(project="Test gpus", name=wandb_name, group=f"{view}")  
    ...
    #   df_val = df_val[:100]
        ...
        model = init_model(accelerator=accelerator ,model_name=modelname, use_pose=use_pose)
        ...
        for epoch in range(1, n_epochs + 1):
            ...
            train_loss = train_epoch(train_loader, model, optimizer, epoch=epoch, accelerator=accelerator)
