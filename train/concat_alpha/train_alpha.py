...
def main(args):
    ...
    def train_epoch(loader, model, optimizer, epoch, mixup=use_mixup):
    ...
    for view in views:
        ...
        folder_name = f"pose-stgcn-{modelname}"
        ...
        if use_wandb:
            # Initialize wandb
            wandb_name = f"train use h_dim {h_dim} alpha {alpha} {folder_name} imgs {img_size} bs {batch_size} {view}"
        ...
        # Setup dataloader
        dataset_train = MyDataset(df=df_train, view=view, img_size=img_size, use_pose=True, data_path=data_path)
        dataset_valid = MyDataset(df=df_val, view=view, img_size=img_size, mode="valid", use_pose=True, data_path=data_path)
        ...
        # Initialize model
        model = Model(model_name=modelname, h_dim=h_dim, alpha=alpha)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--alpha", type=float, default=0.25, help="Alpha rate")
