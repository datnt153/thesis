...
from data_pose import MyDataset
from sklearn.metrics import confusion_matrix
from model_pose import Model
import argparse
...
def main(args):
    ...
    def train_epoch(loader, model, optimizer, epoch, mixup=True):
        ...
        for data in bar:
            pose, target = data
            pose, target = pose.to(device), target.long().to(device)

            loss_func = criterion
            optimizer.zero_grad()
            with torch.cuda.amp.autocast(enabled=use_amp):
                logits, target, target2, lam = model(pose, target)
                if mixup:
            ...
            bar.set_description('loss: %.5f, smth: %.5f, len: %.2d' % (loss_np, smooth_loss, len(target)))
    ...
    for view in views:
        ...
        model_path = f'{dir_model_path}/best.pth'

        Path(dir_model_path).mkdir(parents=True, exist_ok=True)

        if use_wandb:
            # Initialize wandb
            wandb_name = f"train {folder_name} only pose  bs {batch_size} {view}"
        ...
        # Setup dataloader
        dataset_train = MyDataset(df=df_train, view=view, data_path=data_path)
        dataset_valid = MyDataset(df=df_val, view=view, data_path=data_path)
